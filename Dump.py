...
sys.stdout.write('\x1b[1;35m\x1b]2; Created by PRABESH-EXE\x07')
#------------[ INDICATION ]---------------#
id = [] 
tokenku = [] 
uid = []
...
os.system(f'xdg-open https://www.facebook.com/profile.php?id=100074788749098')
# ------------[ MAIN-LOGO ]--------------#
def logo():
	clear()
	print(f'''{white_color} 
...
{50 * '-'}
 >> Author : PRABESH-XWD
 >> Github : https://github.com/PRABESH-EXE
...
def login_check():
	try:
		token = open('/sdcard/PRABESH/dump_login/.token.txt','r').read()
		cok = open('/sdcard/PRABESH/dump_login/.cok.txt','r').read()
...
def login():
	try:
		logo()
		cookie=input(f'{white_color} ENTER YOUR COOKIE: ')
		print(50*"\033[0;97m-")
		open("/sdcard/XYZ/dump_login/.cok.txt", "w").write(cookie)
		with requests.Session() as PRABESH:
			try:
				...
				if '"access_token":' in str(response.headers):
					...
					open("/sdcard/PRABESH/dump_login/.token.txt", "w").write(token)
		...
		os.system("rm -f /sdcard/PRABESH/dump_login/.token.txt")
		os.system("rm -f /sdcard/PRABESH/dump_login/.cok.txt")
...
def menu(_name_,_id_):
	...
	if YounisXyz in ['1']:
		...
		os.system('rm -rf /sdcard/PRABHUU/dump_login/.token.txt')
		os.system('rm -rf /sdcard/PRABESH/dump_login/.cookie.txt')
	...
	for user in uid:
	    try:
	       ...
	       for xyz in url['friends']['data']:
	           try:
	               younisxyz = (PRABESH ['id']+'|'+PRABESH ['name']);open(f'%s'%(file_name),'a').write(xyz['id']+'|'+xyz['name']+'\n')
	               abc = (PRABY['id'])
	               jj = len(id)
...
def dump(fields,cookie,token):
	try:
		print(f'\n For Example /sdcard/Prabesh.txt')
		file_name = input(' Output save file name > ')
...
if __name__=='__main__':
	try:os.mkdir("/sdcard/PRABESH")
	except:pass
	try:os.mkdir("/sdcard/PRABESH/YounisXyz")
	except:pass
	try:os.mkdir("/sdcard/PRABESH/dump_login")
	except:pass
	login_check()
	
#Github: https://github.com/PRABESH-EXE
#Facebook: https://facebook.com/प्रवेश पौडेल
#YOUTUBE: PRABESH GAMING STUDIO 
#Youtube: https://youtube.com/@PRABESH GAMING STUDIO 
#------------------[ << THE END OF CODE >> ]-----------------#
